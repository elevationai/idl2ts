...
module TestModule {
  ...
  struct TestStruct {
    string field;
      ...
      assert(
        moduleCode.includes(
          'IDL:com.example.test/TestModule/TestInterface:1.0',
        ),
      );
      assert(
        moduleCode.includes('IDL:com.example.test/TestModule/TestStruct:1.0'),
      );
    });
  });
...
module TestModule {
  ...
  interface NormalInterface {
    void test();
      ...
      assert(
        moduleCode.includes('IDL:test.org/TestModule/NormalInterface:1.0'),
      );
    });
  });
...
module Components {
  interface ApplicationManager {
    long level();
      ...
      assert(
        moduleCode.includes(
          'IDL:cuss.iata.org/Components/ApplicationManager:1.0',
        ),
      );
      assert(
        moduleCode.includes(
          'static override readonly _repository_id = "IDL:cuss.iata.org/Components/ApplicationManager:1.0"',
        ),
      );
    });
  });

...
export class TypeScriptGenerator {
  ...
  private generateModule(node: AST.ModuleNode): void {
    ...
    this.currentModuleOutput = savedModule
      ? this.modules.get(savedModule) || this.rootModule
      : this.rootModule;
  }
  ...
  private generateOperation(node: AST.OperationNode): void {
    ...
    const outParams = node.parameters.filter((p: AST.ParameterNode) =>
    ...
    if (node.isOneway) {
      ...
      returnType =
        `Promise<{ returnValue: ${returnValueType}; ${outParamTypes} }>`;
    }
  ...
  private generateStructTypeCode(node: AST.StructNode): void {
    ...
    for (let i = 0; i < node.members.length; i++) {
      ...
      this.emit(
        `{ name: "${member.name}", type: ${memberTc} }${
          i < node.members.length - 1 ? ',' : ''
        }`,
      );
    }
    ...
    for (const caseNode of node.cases) {
      if (caseNode.member) {
        const discriminatorValue = caseNode.isDefault
          ? '"default"'
          : caseNode.labels.map((l: string | number | boolean) =>
            JSON.stringify(l)
          ).join(' | ');

        variants.push(
          `{ discriminator: ${discriminatorValue}; ${caseNode.member.name}: ${
  ...
  private generateException(node: AST.ExceptionNode): void {
    ...
    this.emit(
      `constructor(${
        node.members.map((m: AST.MemberNode) =>
          `${m.name}: ${this.mapType(m.type)}`
        ).join(', ')
      }) {`,
    );
    this.indent();
  ...
  private generateStubOperation(
    ...
    const sourceModule =
      (node as AST.OperationNode & ExtendedNode).__sourceModule;
    const sourceInterface =
      (node as AST.OperationNode & ExtendedNode).__sourceInterface;
    const params = node.parameters.map((p: AST.ParameterNode) => {
      ...
      return isOptional
        ? `${paramName}?: ${
          this.mapType(p.type, true, sourceModule, sourceInterface)
        }`
        : `${paramName}: ${
          this.mapType(p.type, true, sourceModule, sourceInterface)
        }`;
    ...
    if (node.isOneway) {
      ...
      // No out parameters - return just the return value
      returnType = `Promise<${
        this.mapType(node.returnType, true, sourceModule, sourceInterface)
      }>`;
    } else if (!hasReturn) {
      // Out parameters but void return - return object with just out params
      const outParamTypes = outParams.map((p: AST.ParameterNode) =>
    ...
    if (node.isOneway) {
      ...
      if (outParams.length === 0) {
        ...
        outParams.forEach((param: AST.ParameterNode, index: number) => {
          const paramIndex = node.parameters.findIndex((p: AST.ParameterNode) =>
            p === param
          );
          const comma = index < outParams.length - 1 ? ',' : '';
          this.emit(
            `${param.name}: request.get_arg(${paramIndex}) as ${
              this.mapType(param.type, true, sourceModule, sourceInterface)
            }${comma}`,
          );
        });
        this.dedent();
  ...
  private generateStubAttribute(
    ...
    const sourceModule =
      (node as AST.AttributeNode & ExtendedNode).__sourceModule;
    const sourceInterface =
      (node as AST.AttributeNode & ExtendedNode).__sourceInterface;
  ...
  private generateServerSkeleton(node: AST.InterfaceNode): void {
    ...
    for (const member of node.members) {
      if (member.kind === 'operation') {
        ...
        // Check if we have out parameters
        const outParams = member.parameters.filter((p: AST.ParameterNode) =>
          p.direction === 'out' || p.direction === 'inout'
        );
